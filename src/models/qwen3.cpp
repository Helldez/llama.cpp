...
llama_model_qwen3::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    // ShardLLM seam: clamp the computed layer range to [pf_start, pf_end). See qwen2.cpp for
    // the rationale; a non-tail stage skips the final norm + lm_head and emits the raw residual.
    int pf_start, pf_end;
